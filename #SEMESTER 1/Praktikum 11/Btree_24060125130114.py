...
# Hitung 1 untuk akar, lalu rekursif pada anak kiri dan kanan pohon
    return NBElmt(Left(PB))+NBElmt(Right(PB))
...
def IsMemberPB(P, X):
    ...
    if Akar(P) == X:
        return True
    return IsMemberPB(Left(P), X) or IsMemberPB(Right(P), X)
...
def IsSkewLeft(P):
    ...
    if isBTDaun(P):
        return True
    if not isBTEmpty(Left(P)) and isBTEmpty(Right(P)):
...
def IsSkewRight(P):
    ...
    if isBTDaun(P):
        return True
    if not isBTEmpty(Right(P)) and isBTEmpty(Left(P)):
        return IsSkewRight(Right(P))
    return False

print(IsSkewRight(P))
#  LevelOfX: Pohon Biner, elemen -> boolean
#   {LevelOfX(P, X) mengembalikan level dari elemen X pada P.}

def LevelOfX(P, X, level=1):
    if isBTEmpty(P):
        return -1
    if Akar(P) == X:
        return level
    left = LevelOfX(Left(P), X, level+1)
    if left != -1:
        return left
    return LevelOfX(Right(P), X, level+1)
